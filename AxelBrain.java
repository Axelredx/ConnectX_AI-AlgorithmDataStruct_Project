...
public class AxelBrain implements CXPlayer{
    ...
    public int iterativeDeepening(CXBoard board, int maxDepth) {
        ...
        for (int depth = 1; depth <= maxDepth; depth++) {
            ...
            int score = alphabeta(board, depth, Integer.MIN_VALUE, Integer.MAX_VALUE, board.currentPlayer(), true);    
    ...
    public int alphabeta(CXBoard board, int depth, int alpha, int beta, int player, boolean maximizingPlayer) {
        ...
        if (maximizingPlayer) {
            ...
            for (int col : board.getAvailableColumns()) {
                ...
                int score = alphabeta(board, depth - 1, alpha, beta, player, false);
                // Undo the move
                board.unmarkColumn();
                // Update max score
        ...
        // If minimizing player's turn
            ...
            for (int col : board.getAvailableColumns()) {
                ...
                board.markColumn(col);
                // Recursively evaluate the move
                int score = alphabeta(board, depth - 1, alpha, beta, player, true);
                // Undo the move
                board.unmarkColumn();
                // Update min score
    ...
    public int evaluate(CXBoard board, int player) {
        ...
        // If the game is won by the player, return a high score
        if (board.gameState() == CXGameState.WINP1 && player == 1 ||
                board.gameState() == CXGameState.WINP2 && player == 2) {
            return 1000;
        }
        // If the game is won by the opponent, return a low score
